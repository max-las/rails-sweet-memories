...
class MemoriesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create memory" do
    assert_difference('Memory.count') do
      post memories_url, params: { memory: { image: @memory.image, note: @memory.note, user: @memory.user } }
    end
  ...
  test "should update memory" do
    patch memory_url(@memory), params: { memory: { image: @memory.image, note: @memory.note, user: @memory.user } }
